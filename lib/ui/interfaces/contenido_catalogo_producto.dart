        ...
        child: Column(
          ...
          children: [
            _PanelFiltrosProducto(
              ...
              onNuevoProducto: () => _guardarProducto(),
            ),
            const SizedBox(height: 18),
            if (_cargando)
              const _EstadoProductos(mensaje: 'Cargando productos...')
            else if (_error != null)
              _EstadoProductos(mensaje: _error!, onReintentar: _cargarProductos)
            else if (_productosFiltrados.isEmpty)
              const _EstadoProductos(mensaje: 'No hay productos para mostrar')
            else
              LayoutBuilder(
